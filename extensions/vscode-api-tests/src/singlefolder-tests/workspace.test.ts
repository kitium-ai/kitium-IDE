...
suite('vscode API - workspace', () => {
	...
	test('openTextDocument, actual casing first', async function () {
		...
		fs.writeFile(uriOne, Buffer.from('one') as Uint8Array, { create: true, overwrite: true });
		fs.writeFile(uriTwo, Buffer.from('two') as Uint8Array, { create: true, overwrite: true });
	...
	test('Support creating binary files in a WorkspaceEdit', async function (): Promise<any> {
		...
		ws.createFile(fileUri, { contents: data as Uint8Array, ignoreIfExists: false, overwrite: false });
	...
	test('encoding: decode', async function () {
		...
		// without setting
		assert.strictEqual(await vscode.workspace.decode(Buffer.from('Hello World') as Uint8Array, uri), 'Hello World');
		assert.strictEqual(await vscode.workspace.decode(Buffer.from('Hellö Wörld') as Uint8Array, uri), 'Hellö Wörld');
		...
		assert.strictEqual(await vscode.workspace.decode(Buffer.from('Hello World') as Uint8Array, uri, { encoding: 'foobar123' }), 'Hello World');
	...
	test('encoding: save text document with a different encoding', async () => {
		...
		await vscode.workspace.fs.writeFile(uri, buf as Uint8Array);
